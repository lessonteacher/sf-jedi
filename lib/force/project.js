...
var ChangeLog = require('./changes');
var Package = require('./package');

class Project {
  static get FOLDER() { return './.force'; }

  constructor(options) {
    this.options = Object.assign({
      src: `${Project.FOLDER}/src`,
      changeDir: `${Project.FOLDER}/changes`,

      // Need more options here that cant think of
      pullOnInit: false
  ...
  isNew() {
    return !fs.existsSync(Project.FOLDER);
  }
  ...
  setup() {
    ...
    return Promise.all([
      fs.outputJson(`${dir}/project.json`, options),
      this._createChangelog(),
      this._createPackage()
    ]).then(() => logger.log('Created project files/folders'));
  }

  // Create the change log
  _createChangelog() {
    let dir = this.options.changeDir;
    return fs.mkdir(dir).then(() => this.changelog = new ChangeLog(dir));
  }
  ...
  _createPackage() {
    ...
    return fs.exists(file).then(exists => exists ?
      fs.readFile(file,'utf-8').then(xml => this.package = Package.fromXml(xml))
      : Promise.resolve(new Package(options.package)).then(pkg => this.package = pkg)
    )
  }
}
