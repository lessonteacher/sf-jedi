...
class Api {
  ...
  _jsForceDeploy(buffer) {
    ...
    return new Promise((resolve, reject) => {
      let asyncResult = this.conn.metadata.deploy(buffer, opt);

      // Print progress
      asyncResult.on('progress', this._logProgress);

      // On complete
      asyncResult.complete(detailed, (err, result) => {
        if (err) {
          reject(err);
        } else {
          resolve(result);
  ...
  _jsForceRetrieve(request) {
    return new Promise((resolve, reject) => {
      let asyncResult = this.conn.metadata.retrieve(request);

      // Print progress
      asyncResult.on('progress', this._logProgress);

      // On complete
      asyncResult.complete((err, result) => {
        if (err) {
          reject(err);
        } else {
          resolve(result);
  ...
  // Logs the async request progress
  _logProgress(request) {
    logger.info('=====> Status: ' + request.state.yellow)
  }
}
...
class RetrieveResults extends Results {
  writeLog() {
    ...
    if(res.success === 'true') {
      message += 'Successful'.bold.green;
    } else {
      message += res.done === 'true' ? 'Failed'.bold.red : 'Not Completed Yet'.bold.red;
    }
...
class DeployResults extends Results {
  writeLog() {
    ...
    if(res.success) {
      message += res.status === 'SucceededPartial' ? 'Partially Succeeded'.bold.yellow : 'Successful'.bold.green;
    } else {
      message += res.done ? 'Failed'.bold.red : 'Not Completed Yet'.bold.red;
    }
